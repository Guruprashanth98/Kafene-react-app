...
const Product = () => {
    ...
    const handleRowCreation = () =>{
        ...
        let _products = []
        if(filters["Expired"]){
            _products = [..._products,...products.filter(item => checkExpiry(item))]
        }
        if(filters["Low Stock"]){
            _products = [..._products,...products.filter(item => checkLowStock(item))]
        }
                ...
                <div className="product-wrapper">
                    <div className="filters-section">
                        ...
                        <div className="filters-wrapper">
                            ...
                            <label className="filter-checkbox"><input type="checkbox" name="products-expired" value="Expired" onChange = {handleFilters} checked={filters["Expired"]}/>Expired</label>
                            <label className="filter-checkbox"><input type="checkbox" name="products-lowstock" value="Low Stock" onChange={handleFilters} checked={filters["Low Stock"]}/>Low Stock</label>
                        </div>
                    </div>
                    <div className="order-list">
                        <table className="order-table">
                            <tbody>
                                ...
                                {filteredProducts.map((item,idx )=> <TableRow key ={idx} product={item}/>)}
                            </tbody>
                        </table>
                    </div>
                </div>
...
const TableRow = ({product}) =>{
    ...
    return (
        <tr className="row">
            <td className="cell-secondary"><Link to={`/product/${id}`} style={{textDecoration:'none', color:'inherit'}}>{id}</Link></td>
            <td className="cell-primary">{medicineName}</td>
            <td className="cell-secondary">{medicineBrand}</td>
            <td className="cell-primary">{dateStr}</td><td className="cell-secondary">{`$${unitPrice}`}</td>
